...
public class Compromissedb {
    ...
    public void sendCompromisso(final Compromisso compromisso, final Calendar calendar){
        final DatabaseReference agendaref = FirebaseDatabase.getInstance().getReference(Tools.user).child(user.getUid()).child(Tools.agenda).child(Tools.formatday(calendar.getTime()));
        agendaref.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (!dataSnapshot.exists()){
                    agendaref.setValue(Tools.formatday(calendar.getTime())).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            DatabaseReference compromissoref = FirebaseDatabase.getInstance().getReference(Tools.user).child(user.getUid()).child(Tools.agenda);
